...
export default function Register({ navigation }) {
  ...
  const getStudents = async () => {
    try {
      const response = await fetch('http://192.168.38.26:5001/get_students');
      if (!response.ok) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      fetch('http://192.168.38.26:5001/add_student_face', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      fetch('http://192.168.38.26:5001/add_student', {
        method: 'POST',
        body: AddNew,
      })
        .then(response => response.json())
