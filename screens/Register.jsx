...
import createFormData from "../utils/uploader";
export default function Register({ navigation }) {
  ...
  //   try {
  //     const response = await fetch('http://192.168.1.17:5001/get_students');
  //     if (!response.ok) {
  ...
  const handleRegister = async () => {
    ...
    Object.keys(registerInfo).map((ele) =>
      formData.append(ele, registerInfo[ele])
    );
    // console.log(formData)
    // const newStudent =await createFormData(file, registerInfo)
    await axios
      .post("/users", newStudent, {
        headers: {
    ...
    //   fetch('http://192.168.1.17:5001/add_student_face', {
    //     method: 'POST',
    //     body: formData,
    //     headers: {
    //       'Content-Type': 'multipart/form-data',
    ...
    //   fetch('http://192.168.1.17:5001/add_student', {
    //     method: 'POST',
    //     body: AddNew,
    //   })
    //     .then(response => response.json())
